...
export default function InfoPanel({ node, edge, nodes = [], onClose, onDelete, readOnly = false }) {
    if (!node && !edge) return null

    const isNode = !!node
    const cfg = isNode ? (NODE_TYPES[node.type] || NODE_TYPES.suspect) : null

    // Resolve edge names from IDs
        ...
        <div className="info-panel fade-in">
            ...
            <div className="info-header">
                <div className="info-header-left">
                    {isNode && (
                        <div className="info-type-badge" style={{ background: cfg.color + '18', border: `1px solid ${cfg.color}40`, color: cfg.color }}>
                            <span>{cfg.icon}</span>
                            <span>{cfg.label}</span>
                        </div>
                    )}
                    {!isNode && (
                        <div className="info-type-badge" style={{ background: 'rgba(0,212,255,0.1)', border: '1px solid rgba(0,212,255,0.3)', color: '#00d4ff' }}>
                            <span>🔗</span>
            ...
            {isNode && node.details && (
                <div className="info-body">
                    {Object.entries(node.details).map(([key, value]) => {
                        // Render Status as a badge instead of plain text
                    ...
                    {node.details.Status && (() => {
                        ...
                        return (
                            <div className="info-status-row">
                                ...
                                <span className="info-status-badge" style={{ background: s.bg, color: s.color, border: `1px solid ${s.border}` }}>
                                    ● {node.details.Status}
                                </span>
                            </div>
                        )
            ...
            <div className="info-actions">
                {!readOnly && (
                    <button className="info-btn info-btn-danger" onClick={() => onDelete(isNode ? node : edge, isNode ? 'node' : 'edge')}>
                        🗑️ Remove
                    </button>
                )}
                <button className="info-btn info-btn-ghost" onClick={onClose}>
                    Close
                </button>
            </div>
        </div>
